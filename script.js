...
// NEW: Secret compressed collection
...
// --- SECRET VAULT UI INJECTION ---
let vaultModal = null;
let vaultHistory = null;
let vaultInput = null;

function buildSecretVaultUI() {
  if (document.getElementById("secretVaultModal")) return; // Already built

  // Create the Secret Modal overlay dynamically
  ...
  vaultModal.innerHTML = `
    <div class="modal-content" style="width: 320px; height: 400px; display: flex; flex-direction: column; background: #07090d; border: 1px solid #ff9f43;">
      <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 10px; margin-bottom: 10px;">
        <h2 style="margin: 0; color: #ff9f43; font-size: 16px;">¢ Secret Vault (Encrypted)</h2>
      ...
      <div style="display: flex; gap: 5px;">
        <input type="text" id="vaultInput" placeholder="Send secure msg..." style="flex: 1; padding: 8px; border-radius: 4px; border: 1px solid #333; background: #111; color: #fff;">
  ...
  document.getElementById("vaultSendBtn").addEventListener("click", async () => {
    ...
    // COMPRESSED BYTE DATA SAVE
    // We use micro-keys: s = sender, m = message, t = time
    await addDoc(secretVaultCollection, {
      s: currentUsername,
      m: text,
      t: Date.now()
    });
...
function checkSecretAccess() {
  ...
  if (currentUsername.includes("¢")) {
    buildSecretVaultUI(); // Ensure UI exists
    if (!existingSecretBtn && headerActions) {
      ...
      // Insert right before the clear chat button
      headerActions.insertBefore(secretBtn, clearChatBtn);
      
      secretBtn.addEventListener("click", () => {
        vaultModal.classList.remove("hidden-modal");
      });
    }
  } else {
    // Destroy button if name changed and ¢ is removed
...
function updateIdentityDisplays() {
  ...
  if (currentUsername) {
    ...
    checkSecretAccess(); // Check for ¢ symbol
  } else {
...
// God purge ONLY clears the main room, not the secret vault
...
async function fetchAiReply(userPrompt) {
  try {
    ...
    const response = await fetch("https://text.pollinations.ai/", {
      ...
      body: JSON.stringify({
        messages: [{ role: "system", content: "You are a helpful, conversational, super fast AI assistant inside a developer chat room." }, { role: "user", content: userPrompt }]
      })
    });
...
if (sendBtn) {
  sendBtn.addEventListener("click", async () => {
    ...
    if (text.toLowerCase() === "/removegod") {
      messageArea.value = "";
      if (!godIsActive) { await sendGodSms("I am already muted this round."); return; }
      const mathQuestion = makeHardQuestion();
      await sendGodSms(`⚡ SYSTEM CHALLENGE ENFORCED: ${mathQuestion}`);
      return;
    }

    await addDoc(messagesCollection, { sender: currentUsername || "Anonymous", senderColor: currentUserColor, message: text, image: selectedImageBase64, time: Date.now(
